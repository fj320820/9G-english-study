...
- "chineseTranslation": Friendly, encouraging Chinese translation of what you just said. Keep it casual and warm. Use Chinese only for this field.`;
    ...
    console.log("[Gemini Notice] /api/chat handled gracefully:", error.message || error);
...
Return a JSON conformant to the response schema. Keep everything supportive and focused on boosting student motivation!`;
    ...
    console.log("[Gemini Notice] /api/report handled gracefully:", error.message || error);
    const isQuotaError = error?.status === 429 || error?.code === 429 || JSON.stringify(error).includes("429") || JSON.stringify(error).includes("quota");
    if (isQuotaError) {
...
- Provide the response in strict JSON format matching the schema below. Keep all tips extremely encouraging and clear for junior high students.`;
    ...
    console.log("[Gemini Notice] /api/expression-coach handled gracefully:", error.message || error);
    const isQuotaError = error?.status === 429 || error?.code === 429 || JSON.stringify(error).includes("429") || JSON.stringify(error).includes("quota");
    if (isQuotaError) {
...
Return your response in strict JSON format matching the schema above. All fields are required.`;
    ...
    console.log("[Gemini Notice] /api/essay-builder handled gracefully:", error.message || error);
    const isQuotaError = error?.status === 429 || error?.code === 429 || JSON.stringify(error).includes("429") || JSON.stringify(error).includes("quota");
    if (isQuotaError) {
...
Return response in strict JSON format.`;
    ...
    // 5. Output full server logs formatted as dynamic recovery
    console.log("[Gemini Notice] /api/writing-coach handled gracefully:", error.message || error);

    try {
      console.log("[Info] Gemini API writing-coach failure, running local writing-coach fallback.");
      const fallbackResult = getSimulatedWritingFallback(taskId, chineseIdeas);
      res.json({ 
        ...fallbackResult, 
        isFallback: true,
      ...
      // If even fallback fails, return error safely
      let errorMessage = "Unknown server-side error during Gemini API request in server.ts.";
      ...
      res.status(500).json({
        error: `Gemini API invocation failed: ${errorMessage}`,
